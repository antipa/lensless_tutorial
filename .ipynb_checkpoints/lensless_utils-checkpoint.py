...
def crop_frac(x,c):
    ...
    crop_size = []
    for n in range(x.ndim):
        crop_size.append(int(np.round(x.shape[n]*c)))
        cstart.append((x.shape[n]-crop_size[n])//2)
    slicer = tuple(slice(cstart[n],cstart[n]+crop_size[n],1) for n in range(len(crop_size)))
    print(slicer)
    return(x[slicer])
...
def admm2d(y, psf_in, tau, niter, options = 'default'):
    ...
    for n in range(niter):
        ...
        ukp_1, ukp_2 = soft_2d_gradient(Lsk1 + alpha2k_1/mu2, Lsk2 + alpha2k_2/mu2, tau[n]/mu2)
...
#             Vmult = 1/(CtC + mu1[n+1])
        ...
        sk = skp;
        cost.append(np.linalg.norm((Hskp)-y,'fro')**2 + tau[n]*TVnorm(skp))
...
def FISTA(grad_func, proj_func, x_init, mu, niter, do_restarting = True):
    ...
    for n in range(niter):    
        ...
        if restart > 0 and do_restarting == True:
            print("restarting")
            t_k = 1
        else: 
            t_k = t_kp
